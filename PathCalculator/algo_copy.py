...
class env3d:
	...
	def __init__(self, x,y,z,xmin,xmax,ymin,ymax,zmin,zmax,hvx,hvy,G,xgmin,xgmax,ygmin,ygmax,zgmin,zgmax):
		self.x = x
		self.y = y
		self.z = z
		self.xmin=xmin
...
#Only hidden obstacles are checked, others are known a-priori
	...
	def sense (self,x,y,r):
		obs_num = len(self.hvx)/4 #four vertices for each rectangular obstacle
		for i in range(1,obs_num+1):
			xomin=self.hvx[4*(i-1)]-r
			xomax=self.hvx[4*(i-1)+2]+r
			yomin=self.hvy[4*(i-1)]-r
			yomax=self.hvy[4*(i-1)+1]+r
			if (x>=xomin) and (x<=xomax) and (y>=yomin) and (y<=yomax):
			
#if the robot is within the sensing range of the obstacle, add it to visibile list
				hx_i= [self.hvx[4*(i-1)],self.hvx[4*(i-1)],self.hvx[4*(i-1)+2],self.hvx[4*(i-1)+2]]
				hy_i= [self.hvy[4*(i-1)],self.hvy[4*(i-1)+1],self.hvy[4*(i-1)+1],self.hvy[4*(i-1)]]
...
class RRT3d:
	...
	def bias (self):
		...
		self.add_node(n,self.xg,self.yg,self.zg) #test goal region
		#find nearest node
		nnear = self.near(n)
		#find new node based on step size
		self.step(nnear,n)
...
# G=RRT3d(nstart,xgmin=xgmin,xgmax=xgmax,ygmin=ygmin,ygmax=ygmax,zgmin=zgmin,zgmax=zgmax,xg=xg,yg=yg,zg=zg)

# #environment instance
# # E=env3d(vx,vy,vz,0,4900,0,3100,0,3000)
# E=env3d(vx,vy,vz,0,100,0,100,0,100,hvx=hvx,hvy=hvy)
# #draw setup
# fig = plt.figure()
